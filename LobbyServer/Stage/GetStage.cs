using EpinelPS.Data;
using EpinelPS.Database; // Added Database reference
    ...
    public class GetStage : LobbyMsgHandler
    {
        ...
        public static NetFieldObjectData CreateFieldInfo(User user, string mapId, out bool BossEntered)
        {
            ...
            if (!found)
            {
                ...
                // FIX: Save the database when a new map chapter is initialized! 
                // This prevents the map from wiping itself clean.
                JsonDb.Save();
