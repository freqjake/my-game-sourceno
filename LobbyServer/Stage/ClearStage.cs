...
using System.Linq; // ADDED LINQ

namespace EpinelPS.LobbyServer.Stage
{
    [PacketPath("/stage/clearstage")] // Adjusted to match your packet path
    public class ClearStage : LobbyMsgHandler
    {
        ...
        public static ResClearStage CompleteStage(User user, int StageId, bool forceCompleteScenarios = false)
        {
            ...
            if (rewardData != null)
            {
                ...
                // CRITICAL FIX: Sync the FinalValue so you get your item popup icons!
                if (response.StageClearReward != null && response.StageClearReward.Currency != null)
                {
                    foreach (var c in response.StageClearReward.Currency)
                    {
            ...
            if (clearedStage.StageType != StageType.Sub && clearedStage.ChapterMod != ChapterMod.Story)
            {
                if (user.MainQuestData.TryGetValue(21, out bool _))
                {
                    ...
                    if (user.OutpostBattleLevel.Exp >= 5)
                    {
                        ...
                        user.AddCurrency(CurrencyType.FreeCash, 100);
                    }
                    else
                    {
            ...
            // ---------------------------------------------------------
            // CRITICAL FIXES BELOW: UNLOCKS THE MAP AND CHAPTER BUTTONS
            // ---------------------------------------------------------

            // 1. Add to the physical map completion
            if (!user.FieldInfoNew[stageMapId].CompletedStages.Contains(StageId))
            {
                user.FieldInfoNew[stageMapId].CompletedStages.Add(StageId);
            }

            // 2. Open the gate to the next chapter if this is a boss
            if (clearedStage.StageCategory == StageCategory.Boss)
            {
                user.FieldInfoNew[stageMapId].BossEntered = true;
            }

            // ---------------------------------------------------------

            // CRITICAL: Save the database so the map stays unlocked!
            JsonDb.Save();
            return response;
        }
