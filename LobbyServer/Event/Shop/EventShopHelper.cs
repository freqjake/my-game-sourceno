...
namespace EpinelPS.LobbyServer.Event.Shop
{
    public static class EventShopHelper
    {
            ...
            ref ResEventShopMultipleBuyProduct response)
        {
            ...
            // Handle Item Subtraction (Fix for UI Desync)
            foreach (int tid in totalItemPrice.Keys)
            {
                ...
                item.Count -= totalItemPrice[tid];

                // 2. Add to response BEFORE removing from user.Items
                // This ensures the client gets the Isn and Count: 0 to clear the UI slot
                ...
                // 3. Physically remove it from the server's database list if empty
                if (item.Count <= 0)
                {
