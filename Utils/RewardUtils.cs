    ...
    public class RewardUtils
    {
        ...
        public static void AddSingleObject(User user, ref NetRewardData ret, int rewardId, RewardType rewardType, int rewardCount)
        {
            ...
            else if (rewardType == RewardType.ProfileCardObject)
            {
                ...
                if (!user.Items.Any(x => x.ItemType == rewardId))
                {
            ...
            // --------------------------------
            // FIX: Safely capture Pass Points and prevent loop overwriting
            else if (rewardType.ToString().ToLower().Contains("pass") || (int)rewardType == 15 || (int)rewardType == 16)
            {
                // 1. Send the item data to the client so the UI shows the correct icon
                ret.Item.Add(new NetItemData()
                {
                    Count = rewardCount,
                    Tid = rewardId,
                ...
                // 2. Add the points to the user's actual pass progress
                if (user.UserPassInfo != null)
                {
                    foreach (var pass in user.UserPassInfo.Values)
                    {
                        pass.PassPoint += (int)rewardCount;

                        ret.PassPoint = new NetPassPointData()
                        {
                            Value = pass.PassId,
                            FinalValue = (int)pass.PassPoint
                        };

                        // CRITICAL FIX: Stop the loop after updating the first pass!
                        // This prevents it from accidentally sending the wrong PassId to the client.
                        break;
                    }
                }
            }
