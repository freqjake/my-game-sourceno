...
public class DbStickerPlacement
{
...
public class DbProfileCardLayout
{
    public int BackgroundId { get; set; } = 101001;
    public bool ShowCharacterSpine { get; set; } = true;
    public List<DbStickerPlacement> StickerPlacements { get; set; } = new();
}
public class User
{
    // User info
    ...
    // Replace the old ProfileCardLayout line with this one:
    public DbProfileCardLayout ProfileCardLayout = new();
    ...
    // Tracing the Stars (TTS) Data
    public long TotalTtsPlayTime = 0;
    public long TotalTtsScore = 0;
    public Dictionary<int, long> TtsSongHighScores = new();
    public List<int> ClearedEventStages { get; set; } = new();
    public Dictionary<int, List<int>> ClearedEventMissions { get; set; } = new();
    public List<int> ClaimedLycorisTasks { get; set; } = new();
    public List<int> ClearedLycorisMissions { get; set; } = new();
    // 1. Remembers which map tutorials/notices you have read
    public List<int> ViewedEventNotices { get; set; } = new();

    // 2. Remembers which switches you pressed or barriers you broke
    public List<string> SavedFieldObjects { get; set; } = new();
    public List<string> OpenedPasswordDoors { get; set; } = new();
    // Change this from <string> to <int>
    public List<int> AcquiredFieldCollectibles { get; set; } = new();
    // Change <string> to <int>
    public List<int> AcquiredFieldPasswords { get; set; } = new();
    public List<int> UnlockedFieldPasswordDoors { get; set; } = new();
    // Game data
    ...
    public List<int> CounseledCharacterIds { get; set; } = new();
    ...
    public void ResetDataIfNeeded()
    {
        ...
        if (ShouldResetUser())
        {
            ...
            // ==========================================
            // ADD YOUR COUNSEL RESET LOGIC HERE
            // ==========================================
            foreach (var bond in this.BondInfo)
            {
                bond.CanCounselToday = true;
                // Optional: You can also reset the count to 0 if your client relies on it
                // bond.CounseledCount = 0; 
            }
            // ==========================================
